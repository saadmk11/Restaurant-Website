...
urlpatterns = [
    ...
    url(r'^category/details/(?P<slug>[\w-]+)/$', cat_detail, name="cat_detail"),
    url(r'^details/(?P<slug>[\w-]+)/$', menu_detail, name="menu_detail"),
    #menu api
    url(r'^api/$', MenuListAPIView.as_view(), name="menu_list_api"),
    url(r'^api/create/$', MenuCreateAPIView.as_view(), name="menu_create_api"),
    url(r'^api/(?P<slug>[\w-]+)/$', MenuRetrieveAPIView.as_view(), name="menu_detail_api"),
    url(r'^api/(?P<slug>[\w-]+)/update/$', MenuRetrieveUpdateAPIView.as_view(), name="menu_update_api"),
